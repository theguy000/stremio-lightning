...
impl MacosArch {
    pub fn parse(value: &str) -> Result<Self> {
        match value.to_ascii_lowercase().as_str() {
            ...
            other => Err(format!(
                "unsupported macOS architecture '{other}'. Use arm64 or x86_64"
            )
            .into()),
        }
    }
...
pub fn package_macos(arch: MacosArch) -> Result<()> {
    ...
    println!(
        "==> Building native macOS shell crate ({})...",
        arch.name()
    );
    run_program(
        "cargo",
...
fn verify_bundle(
    ...
    required_file(&bundle.join("Contents/Info.plist"), "run: cargo xtask package-macos")?;
    required_file(
    ...
    if bundled_dylibs.is_empty() {
        return Err("no dylibs were bundled into Contents/Frameworks; expected at least libmpv".into());
    }
